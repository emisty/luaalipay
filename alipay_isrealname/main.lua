...
function saveImei(imei,nowtime)
	body,c,l,h = http.request(geturl..'SaveImei2?imei='..imei.."&times="..nowtime)
end

saveImei(imei,nowtime)
mSleep(2000)
toast("请根据当前手机id"..imei.."去数据库编辑号段",5)
...
function getPhone(imei)
	body,c,l,h = http.request(geturl..'GetPhone2?imei='..imei.."&times="..nowtime)
	...
	if ss == "0" then
		...
		mSleep(3000)
		toast("请根据当前手机id"..imei.."去数据库编辑号段",2)
		getPhone(imei)	
	 else
		pre_phone = ss
	end
...
toast(pre_phone.."号段成功绑定",1)


--
function SaveCheckPhone(account, isrealname, ischeck)
	body,c,l,h = http.request(geturl..'SaveCheckPhone?account='..account.."&isrealname="..isrealname.."&ischeck="..ischeck)
end
...
function isExistAddFriend()
	...
	elseif pixel == "10801920" then			
		return isPositionInb(740,1169,"0x3c8bc3",759,1175,"0xffffff",768,1171,"0x198be1",778,1175,"0x1f89d5",786,1181,"0x1592ee",813,1156,"0x248bce",842,1178,"0xf7ffff",845,1183,"0x168de7")
	elseif pixel == "15362084" then
...
function addFriend(phone,mes)
	...
	SaveCheckPhone(phone,isreal,3)

	--如果存在加好友三个字
	--if isExistAddFriend() then
		--点击加好友
		--accountClickAdd()
...
function flowAddFriend()
	...
	while iscc do 
		space = ""
		for i=1,11-#pre_phone-#tostring(nowadd) do
			space = space.."0"
		end
		toast(space,2)
		if #tostring(nowadd) <= 11-#pre_phone then
			nowaddS = space .. tostring(nowadd)
			toast(pre_phone..nowaddS,2)
			addFriend(pre_phone..nowaddS,"我喜欢你")
			nowadd = nowadd+1
		else
			iscc = false
			toast("结束了", 1)
